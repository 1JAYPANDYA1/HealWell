...
const SkinChecker = () => {
  ...
  const handleLanguageChange = async (e) => {
    ...
    if (cures) {
      await translateCures(cures, newLanguage);
    }
  };

  const translateCures = async (text, targetLang) => {
    setPrecautionLoading(true);
    try {
      const response = await axios.post(`${VITE_API_URL}/patient/translate`, {
        text,
        targetLang
      },{withCredentials:true});
      setCures(response.data.translatedText);
    setPrecautionLoading(false);
    } catch (error) {
      console.error('Translation error:', error);
      setError("Error: Unable to translate content.");
    }
  };
  ...
  const fetchCureFromGemini = async (disease) => {
    ...
    try {
      ...
      if (cureList) {
        const curesText = cureList.join("\n");
        if (selectedLanguage !== 'en') {
          await translateCures(curesText, selectedLanguage);
        } else {
          setCures(curesText);
        }
      } else {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto bg-white shadow-lg rounded-xl border border-gray-200 flex flex-col lg:flex-row gap-4 lg:gap-6 min-h-[450px] h-full w-full overflow-y-auto" data-aos="zoom-in-up">
      ...
      {/* Right Side - Treatment Suggestions with Language Selection */}
      <div className="w-full lg:w-1/2 bg-green-50 p-4 sm:p-6 border border-green-200 rounded-md flex-1 flex flex-col self-stretch h-full">
        <div className="flex justify-between items-center mb-4">
          ...
          <select
            ...
            className="px-3 py-1 border border-green-300 rounded-md bg-white text-green-700 focus:outline-none focus:ring-2 focus:ring-green-500"
          >
